;;; core/vonfry-keybinding.el --- -*- lexical-binding: t -*-
;;
;; This file doesn't set a lot of keybindinds, because the plugins are setting
;; in modules and they cannot be set here.
;; If using hooks, I think it isn't beautiful enough. So the file only defines
;; some keys which are bounded to use.
;;
;; The name with evil means that the keybind use with <leader> in evil-normal. I
;; don't use `general-default-prefix` to set leader because there are some
;; normal bind without leader.
;;
;; All local map set for major or minor mode map by default. Only if the minor
;; mode hasn't a map, such as robe, using evil minor mode map instead which use
;; a hook to add into local map.
;; All keybind use general.el to manage.
;;
;; All keys combine with leader when setting, this can have a more structed
;; definitation.
